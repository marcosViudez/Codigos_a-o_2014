		...
		if(reCheck || triggerChildrenColliderScript == null){ //If children collider or root collider are hit, process the bullet hit.
			...
			if(hit.rigidbody != null){ //Apply force.
				hit.rigidbody.AddForceAtPosition(velocity* 1.5, hit.point);
				 ...
				 // ****** marcos codigo para matar arañas que vienen a por ti ****
			
				 // llamar a un metodo
  				//Debug.Log(hit.rigidbody.name); // escribe el que colisiono
  				hit.rigidbody.GetComponent.<Animation>().Play("death"); // anima a la que mato
  				//hit.rigidbody.active=false; // las hace desaparecer pero no destruye
  				hit.rigidbody.GetComponent("iaArana").viva(); // accede al metodo
				 // ***************************************************************
